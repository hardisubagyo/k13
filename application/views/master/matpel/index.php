							...
							<tr>
								...
								<td>
									<div class="list-icons list-icons-extended">
										<button type="button" class="btn btn-outline bg-brown text-brown-800 btn-icon ml-2 Edit_Matpel" data-toggle="modal" data-target="#Edit_Matpel" data-id="<?php echo $item->id_matpel; ?>" data-backdrop="static" data-keyboard="false">
											<i class="icon-pencil3"></i>
										</button>
										<button type="button" class="btn btn-outline bg-danger text-danger-800 btn-icon ml-2" onclick="ConfirmDelete(<?php echo $item->id_matpel; ?>)">
...
<div id="Edit_Matpel" class="modal fade" tabindex="-1">
	<div class="modal-dialog">
		<div class="modal-content">
			<form action="<?php echo site_url('Matpel/ubah'); ?>" method="post">
				<div class="modal-header">
				...
				<div class="modal-body">				
					<div class="form-group">
						<label>Nama Mata Pelajaran</label>
						<input type="text" name="nama_matpel" id="nama_matpel" class="form-control" placeholder="Nama Mata Pelajaran">
					</div>

					<div class="form-group">
						<label>Jenis Mata Pelajaran</label>
						<select class="form-control" name="id_jenis_matpel" id="id_jenis_matpel">
							<?php foreach($jenis_matpel as $item){ ?>
								<option value="<?php echo $item->id_jenis_matpel; ?>"><?php echo $item->nama_jenis_matpel; ?></option>
							<?php } ?>
						</select>
...
<script type="text/javascript">
	function ConfirmDelete(val){
    	if (confirm("Hapus Data ?"))
      	location.href='Matpel/Delete/'+val;
  	}
  	...
  	$(document).on("click", ".Edit_Matpel", function () {
	    ...
	    $.ajax({
		    ...
		    url: "<?php echo site_url('Matpel/Edit'); ?>",
		    dataType: "json",
		    success: function(data){
		        if(data.status == '1'){
		        	var get = JSON.parse(data.data);
		        	$('#nama_matpel').val(get.nama_matpel);
		        	$('#id_matpel').val(Id);
		        	document.getElementById("id_jenis_matpel").value = get.id_jenis_matpel;
		        }else{
